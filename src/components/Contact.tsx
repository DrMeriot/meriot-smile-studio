      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-muted-foreground mb-3">
                      {adresse.split(",").map((part: string, i: number) => (
                        <span key={i}>{part.trim()}{i === 0 && <br />}</span>
                      ))}
                    </p>
                    <a href={mapsUrl} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" size="sm">Voir sur Google Maps</Button>
                    </a>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="shadow-soft">
              <CardContent className="pt-6">
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-accent/10 rounded-lg"><Phone className="h-6 w-6 text-accent" /></div>
                  <div className="flex-1">
                    <h3 className="font-semibold text-lg mb-2">Téléphone</h3>
                    <p className="text-muted-foreground mb-3">{tel}</p>
                    <a href={telHref}>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2 text-sm">
                      {horaires.map(({ jour, val }: { jour: string; val: string }) => (
            ...
            <Card className="shadow-soft">
              <CardContent className="pt-6">
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-secondary rounded-lg"><Train className="h-6 w-6 text-primary" /></div>
                  <div className="flex-1">
        ...
        <div className="mt-16 max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {zones.map(({ secteur, villes }: { secteur: string; villes: string[] }) => (
              <div key={secteur} className="bg-card rounded-xl p-5 shadow-soft">
                <h4 className="font-semibold mb-3 pb-2 border-b">{secteur}</h4>
                <ul className="space-y-1 text-sm text-muted-foreground">
                  {villes.map((ville: string) => (<li key={ville}>{ville}</li>))}
                </ul>
              </div>
            ))}
          </div>
