...
# Use the operating system's trust store (Windows certificate store) for SSL
# verification. This makes HTTPS verification succeed when an antivirus or
# corporate proxy (e.g. Avast) intercepts TLS with its own root certificate,
# which would otherwise cause CERTIFICATE_VERIFY_FAILED with the bundled certifi CAs.
try:
    import truststore
    truststore.inject_into_ssl()
except Exception as _e:  # truststore missing or failed: fall back to default certifi behaviour
    print(f"[gsc-server] truststore not active ({_e}); using default SSL certs", file=sys.stderr)
